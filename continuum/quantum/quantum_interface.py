...
class QuantumInterface:
    ...
    async def _run_ibm_quantum(self, qc: Any, shots: int) -> Dict[str, int]:
        ...
        Connects via QiskitRuntimeService using the stored IBM token, selects the
        least-busy real backend, transpiles, and runs via SamplerV2.  Falls back
        to AerSimulator if the runtime package is missing or the job fails.
        """
        from qiskit_aer import AerSimulator
        ...
        try:
            ...
            logger.info(f"IBM Quantum job completed on {ibm_backend.name}")
            return counts
