...
class CCTTrainer:
    ...
    def evaluate(self, dataset: CCTDataset, batch_size: int = 32) -> Dict[str, Any]:
        """
        Evaluate the model on link prediction and self-state metrics.

        Computes accuracy, precision, recall, F1 on link prediction,
        plus resonance and coherence from the model's self-perception.
        ...
        if not hasattr(self, 'link_proj'):
            # Infer hidden_dim from a dummy forward pass
            with torch.no_grad():
                dummy_ctx = torch.zeros(1, 2, 128).to(self.device)
                dummy_state = global_state.unsqueeze(0)
                out = self.model(
                    node_features=node_features,
                    edge_index=edge_index,
...
def main():
    ...
    if args.evaluate:
        if model_path.exists():
            ...
            print(f"  Resonance : {metrics['resonance']:.4f}  (target: π×φ = {PI_PHI:.4f})")
