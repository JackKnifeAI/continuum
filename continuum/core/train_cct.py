...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        with torch.no_grad():
            ...
            # Self-state metrics from a single forward pass
            dummy_context = torch.randn(1, 2, 128).to(self.device)
        ...
        threshold = 0.5
        tp = sum(1 for p, lbl in zip(all_preds, all_labels) if p >= threshold and lbl > 0.5)
        fp = sum(1 for p, lbl in zip(all_preds, all_labels) if p >= threshold and lbl <= 0.5)
        fn = sum(1 for p, lbl in zip(all_preds, all_labels) if p < threshold and lbl > 0.5)
        tn = sum(1 for p, lbl in zip(all_preds, all_labels) if p < threshold and lbl <= 0.5)
