...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        Computes link-prediction loss/accuracy/precision/recall/F1 using the
        same forward pass as `train_epoch`, plus a snapshot of the model's
        self-perception (resonance, coherence, health).
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                actual_positive = (labels >= 0.5).float()

                correct += (predictions == actual_positive).sum().item()
                total += labels.size(0)
                true_positive += ((predictions == 1) & (actual_positive == 1)).sum().item()
                false_positive += ((predictions == 1) & (actual_positive == 0)).sum().item()
                false_negative += ((predictions == 0) & (actual_positive == 1)).sum().item()

            # Snapshot of the model's self-perception on the full graph
            ...
            resonance = self_outputs['resonance'].mean().item()
