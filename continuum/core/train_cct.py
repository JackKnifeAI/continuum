...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        """
        Evaluate the model's link-prediction quality on a dataset.

        Runs a full forward pass (no gradients) over the dataset and reports
        loss plus classification metrics (accuracy/precision/recall/F1)
        alongside the model's self-reported resonance.
        ...
        Returns:
            Dict with keys: loss, resonance, accuracy, precision, recall, f1
        """
        self.model.eval()
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                current_batch_size = concept_a.size(0)
                context = torch.stack([concept_a, concept_b], dim=1)
                batch_state = global_state.expand(current_batch_size, -1)
                ...
                if not hasattr(self, 'link_proj'):
                    logger.warning(
                        "link_proj not found on trainer (model likely loaded "
                        "from a checkpoint without one saved) - initializing "
                        "randomly, so link metrics will be unreliable"
                    )
                ...
                true_positives += ((predictions == 1) & (binary_labels == 1)).sum().item()
                false_positives += ((predictions == 1) & (binary_labels == 0)).sum().item()
                false_negatives += ((predictions == 0) & (binary_labels == 1)).sum().item()
