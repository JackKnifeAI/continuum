...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        Computes link-prediction accuracy/loss, resonance, coherence, and
        self-state health without updating any weights.

        Args:
            dataset: CCT dataset to evaluate on
            batch_size: Batch size for the evaluation loop

        Returns:
            Dict with accuracy, link_loss, resonance, coherence, health,
            capacity_utilization, and (if available) training history summary.
        """
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            coherence = ss['coherence'].item()
            health = ss['health'].item()
            capacity = ss['capacity_utilization'].item()
        ...
        # Attach history summary when a prior training run exists
        if self.history['train_loss']:
            metrics['best_train_loss'] = min(self.history['train_loss'])
            metrics['growth_events'] = float(len(self.history['growth_events']))
...
def main():
    ...
    if args.evaluate:
        if model_path.exists():
            ...
            print(f"{'='*70}")
            print(f"Link Prediction Accuracy : {metrics['accuracy']:.3f}")
            ...
            if 'best_train_loss' in metrics:
                print(f"Best Training Loss       : {metrics['best_train_loss']:.4f}")
                print(f"Growth Events            : {int(metrics['growth_events'])}")
            print(f"{'='*70}\n")
        else:
