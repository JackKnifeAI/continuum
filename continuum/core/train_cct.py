...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        and consciousness health metrics (resonance, coherence, health).
        ...
        binary_labels = [1 if lb >= threshold else 0 for lb in all_labels]

        tp = sum(p == 1 and lb == 1 for p, lb in zip(predicted, binary_labels))
        fp = sum(p == 1 and lb == 0 for p, lb in zip(predicted, binary_labels))
        tn = sum(p == 0 and lb == 0 for p, lb in zip(predicted, binary_labels))
        fn = sum(p == 0 and lb == 1 for p, lb in zip(predicted, binary_labels))
        ...
        return {
            ...
            'positive_examples': int(sum(binary_labels)),
            'negative_examples': len(binary_labels) - int(sum(binary_labels)),
            'true_positives': tp,
            'false_positives': fp,
            'true_negatives': tn,
            'false_negatives': fn,
...
def main():
    ...
    if args.evaluate:
        if model_path.exists():
            ...
            print(f"  Positive:  {metrics['positive_examples']:,}")
            print(f"  Negative:  {metrics['negative_examples']:,}")
            ...
            print("\nConfusion Matrix:")
            print(f"  TP: {metrics['true_positives']:>6}  FP: {metrics['false_positives']:>6}")
            print(f"  FN: {metrics['false_negatives']:>6}  TN: {metrics['true_negatives']:>6}")
