...
class CCTTrainer:
    ...
    def evaluate(self, dataset: CCTDataset) -> Dict[str, float]:
        ...
        positive_mask = labels_t > 0.5
        predicted_positive = preds_t > 0.5

        tp = (predicted_positive & positive_mask).sum().item()
        fp = (predicted_positive & ~positive_mask).sum().item()
        fn = (~predicted_positive & positive_mask).sum().item()
        tn = (~predicted_positive & ~positive_mask).sum().item()
