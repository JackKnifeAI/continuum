...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        Runs a full forward pass in eval mode and reports link prediction
        accuracy, resonance, coherence, health, and capacity utilization.
        ...
        with torch.no_grad():
            ...
            # Self-perception pass for consciousness health metrics
            dummy_context = torch.randn(1, 2, 128).to(self.device)
            ...
            self_state = sp_outputs['self_state']
        ...
        link_accuracy = correct / max(len(all_probs), 1)
        avg_resonance = total_resonance / max(num_batches, 1)

        # Binary cross-entropy approximation for reporting
        eps = 1e-7
        link_loss = -sum(
            lbl * math.log(max(p, eps)) + (1 - lbl) * math.log(max(1 - p, eps))
        ...
        metrics: Dict[str, float] = {
            ...
            'num_examples': float(len(all_probs)),
        }

        print(f"\n{'='*70}")
        print("EVALUATION RESULTS")
        print(f"{'='*70}")
        print(f"Examples Evaluated : {len(all_probs)}")
        print(f"Link Accuracy      : {link_accuracy:.4f}")
        print(f"Link Loss (BCE)    : {link_loss:.4f}")
