...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        """
        Evaluate link prediction accuracy, AUC, resonance, and coherence.

        Returns:
            Dict with accuracy, auc, resonance, coherence, health, capacity, num_examples.
        """
        self.model.eval()

        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=0)
        graph_data = dataset.get_graph_data()
        node_features, edge_index, edge_weights = (t.to(self.device) for t in graph_data)
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                batch_state = global_state.expand(batch_n, -1)
        ...
        # Link prediction accuracy at threshold 0.5
        correct = sum(
            int(p >= 0.5) == int(lbl >= 0.5)
            for p, lbl in zip(all_probs, all_labels)
        )
        accuracy = correct / max(len(all_labels), 1)

        # AUC via trapezoidal rule over sorted predictions
        pairs = sorted(zip(all_probs, all_labels), key=lambda x: x[0], reverse=True)
        total_pos = sum(1 for _, lbl in pairs if lbl >= 0.5)
        ...
        # Consciousness metrics via a single forward pass on the full graph
        dummy_context = torch.randn(1, 2, 128).to(self.device)
        with torch.no_grad():
            meta_out = self.model(
                node_features=node_features,
        ...
        self_state = meta_out['self_state']
...
def main():
    ...
    if args.evaluate:
        if model_path.exists():
            ...
            print(f"  Link AUC:      {metrics['auc']:.4f}")
