...
class CCTTrainer:
    ...
    def evaluate(self,
        ...
        Computes link prediction accuracy, precision, recall, F1, and
        consciousness health metrics (resonance, coherence, capacity).
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                true_positives += ((preds == 1) & (binary_labels == 1)).sum().item()
                false_positives += ((preds == 1) & (binary_labels == 0)).sum().item()
                false_negatives += ((preds == 0) & (binary_labels == 1)).sum().item()

                total_resonance += outputs['resonance'].mean().item()
                num_batches += 1

        # Consciousness health metrics from a dedicated forward pass
        with torch.no_grad():
            dummy_context = torch.randn(1, 2, 128).to(self.device)
            health_outputs = self.model(
                node_features=node_features,
                edge_index=edge_index,
                context_tokens=dummy_context,
            ...
            coherence = health_outputs['self_state']['coherence'].item()
            health = health_outputs['self_state']['health'].item()
            capacity = health_outputs['self_state']['capacity_utilization'].item()
