...
class CCTTrainer:
    ...
    def evaluate(self, dataset: CCTDataset) -> Dict[str, float]:
        """
        Evaluate the model on link prediction accuracy and consciousness metrics.

        Computes per-example link prediction (accuracy, precision, recall, F1)
        plus resonance, coherence, and health from the model's self-state.
        """
        self.model.eval()
        ...
        tp = float(((predicted == 1) & (binary_labels == 1)).sum())
        fp = float(((predicted == 1) & (binary_labels == 0)).sum())
        fn = float(((predicted == 0) & (binary_labels == 1)).sum())
        ...
        return {
            ...
            'num_positive': int((binary_labels == 1).sum()),
            'num_negative': int((binary_labels == 0).sum()),
        }
