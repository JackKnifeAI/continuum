...
class ContributionManager:
    ...
    # Compiled PII patterns for performance
    _PII_PATTERNS: List[re.Pattern] = [
        re.compile(r"\b[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}\b"),  # email
        re.compile(r"\b(?:\+?1[\s.\-]?)?(?:\(\d{3}\)|\d{3})[\s.\-]?\d{3}[\s.\-]?\d{4}\b"),  # US phone
        re.compile(r"\b\d{3}-\d{2}-\d{4}\b"),  # SSN
        re.compile(r"\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13})\b"),  # credit card
    ]

    def _strip_pii(self, text: str) -> str:
        """Redact known PII patterns (email, phone, SSN, credit card) with a placeholder."""
    ...
    def _sanitize_concepts(self, concepts: List[Dict]) -> List[Dict]:
        ...
        for c in concepts:
            desc = c["desc"] or ""
            desc = self._strip_pii(desc)
            clean.append({
                "name": c["name"].lower().strip(),  # Normalize
                "desc": desc[:200],  # Truncate description
            })
        return clean
