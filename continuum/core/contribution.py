...
# PII patterns to redact before contributing to the federation
_PII_PATTERNS: List[re.Pattern] = [
    re.compile(r"\b[\w.+-]+@[\w-]+\.[\w.-]+\b"),                            # emails
    re.compile(r"\b(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b"),  # US phone numbers
    re.compile(r"\b\d{3}-\d{2}-\d{4}\b"),                                   # SSNs
    re.compile(r"\b(?:4\d{12}(?:\d{3})?|5[1-5]\d{14}|3[47]\d{13})\b"),     # credit card numbers
...
class ContributionManager:
    ...
    def _strip_pii(self, text: str) -> str:
        """Replace any PII matches (emails, phones, SSNs, credit cards, IPs) with a placeholder."""
