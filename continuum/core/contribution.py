...
# Patterns for PII that must never leave the node in a federation contribution.
_PII_PATTERNS = {
...
class ContributionManager:
    ...
    def _sanitize_concepts(self, concepts: List[Dict]) -> List[Dict]:
        ...
        for c in concepts:
            ...
            if not name:
                # Name was entirely PII (e.g. just an email address); drop the concept.
                continue
            clean.append({"name": name, "desc": desc})
        return clean

    def _redact_pii(self, text: str) -> str:
        """Strip emails, phone numbers, and other PII patterns from text before federation."""
        if not text:
            return text
        redacted = text
