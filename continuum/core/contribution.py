...
# PII detection patterns: match emails and phone numbers for redaction
...
class ContributionManager:
    ...
    def _sanitize_concepts(self, concepts: List[Dict]) -> List[Dict]:
        ...
        for c in concepts:
            ...
            # Skip concepts whose name is itself a PII token (e.g. an email address)
            if _contains_pii(name):
                logger.debug("Dropping PII concept: %s", name[:20])
                continue
