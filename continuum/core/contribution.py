...
# PII patterns: email, phone (intl/US), SSN, credit card
...
class ContributionManager:
    ...
    def _remove_pii(text: str) -> str:
        """Replace emails, phone numbers, SSNs, and credit cards with [REDACTED]."""
    ...
    def _sanitize_concepts(self, concepts: List[Dict]) -> List[Dict]:
        ...
        clean = []
        for c in concepts:
            name = self._remove_pii(c["name"].lower().strip())
            desc = self._remove_pii(c["desc"][:200]) if c["desc"] else ""
