...
# PII scrubbing patterns applied before any data leaves the node
_PII_PATTERNS: list[tuple[re.Pattern, str]] = [
    (re.compile(r"[\w.+\-]+@[\w\-]+\.[\w.\-]+"), "[EMAIL]"),
    # E.164 / common North-American / international phone formats
    (re.compile(r"\+?1?\s*[\-.]?\s*\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4}"), "[PHONE]"),
    # Generic digit runs that look like phone numbers (7–15 digits, possibly separated)
    (re.compile(r"\b\d[\d\s\-().]{6,}\d\b"), "[PHONE]"),
]


def _scrub_pii(text: str) -> str:
    """Replace known PII patterns (emails, phone numbers) with safe placeholders."""
...
class ContributionManager:
    ...
    def _sanitize_concepts(self, concepts: List[Dict]) -> List[Dict]:
        """Remove PII and tenant info from concept names and descriptions."""
        clean = []
        for c in concepts:
