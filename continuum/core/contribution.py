...
# PII detection patterns — used by _sanitize_concepts to redact before federation upload
_PII_PATTERNS = [
    re.compile(r"\b[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}\b"),  # email
    re.compile(r"\b(?:\+?1[\s\-.]?)?\(?\d{3}\)?[\s\-.]?\d{3}[\s\-.]?\d{4}\b"),  # phone (NA formats)
...
class ContributionManager:
    ...
    def _sanitize_concepts(self, concepts: List[Dict]) -> List[Dict]:
        ...
        for c in concepts:
            ...
            # Strip PII (emails, phone numbers, SSNs) from both fields
            for pattern in _PII_PATTERNS:
                name = pattern.sub("[REDACTED]", name)
                desc = pattern.sub("[REDACTED]", desc)
            # Skip concepts whose names were entirely PII
            if name in ("[REDACTED]", ""):
                continue
