...
class WebSocketHandler:
    ...
    async def _get_current_state(self, tenant_id: str) -> dict:
        ...
        Returns:
            Dictionary with current state including sync and memory stats.
        """
        # Get sync stats
        ...
        # ConsciousMemory.get_stats() is synchronous SQLite I/O, so run in executor
        # to avoid blocking the async event loop.
        try:
            loop = asyncio.get_running_loop()
            memory_stats = await loop.run_in_executor(
                None,
                lambda: ConsciousMemory(tenant_id=tenant_id).get_stats(),
            )
