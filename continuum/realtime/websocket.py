...
class WebSocketHandler:
    ...
    async def _get_current_state(self, tenant_id: str) -> dict[str, Any]:
        ...
        Returns:
            Dictionary with current state information including memory and sync stats
        """
        # Get sync stats
        ...
        # Fetch memory stats from the storage backend; run synchronous DB call off
        # the event loop to avoid blocking.
        try:
            memory_stats = await asyncio.to_thread(
                self._fetch_memory_stats, tenant_id
            )
    ...
    @staticmethod
    def _fetch_memory_stats(tenant_id: str) -> dict[str, Any]:
        """Synchronous helper to query ConsciousMemory stats (runs in thread pool)."""
