...
class WebSocketHandler:
    ...
    async def _get_current_state(self, tenant_id: str) -> Dict[str, Any]:
        ...
        # Fetch memory stats from the ConsciousMemory backend.
        # Run in executor because ConsciousMemory.get_stats() performs synchronous DB IO.
        if _MEMORY_AVAILABLE:
            try:
                loop = asyncio.get_event_loop()
                memory = ConsciousMemory(tenant_id=tenant_id)
                memory_stats = await loop.run_in_executor(None, memory.get_stats)
            ...
            state["memory_stats"] = {"error": "memory module not installed"}
