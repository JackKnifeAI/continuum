...
class WebSocketHandler:
    ...
    async def _get_current_state(self, tenant_id: str) -> dict:
        ...
        Returns:
            ...
            from the ConsciousMemory backend (entities, messages, attention
            links, compound concepts, cache stats).
        """
        # Get sync stats
        ...
        # Add memory stats from ConsciousMemory backend
        try:
            memory = ConsciousMemory(tenant_id=tenant_id)
            state["memory_stats"] = await memory.aget_stats()
        except Exception as e:
