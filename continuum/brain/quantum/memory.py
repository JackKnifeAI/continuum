...
# Patterns that signal a decision or commitment being made
_DECISION_PATTERNS = [
    r"\b(i will|we will|i'll|we'll|i'm going to|we're going to|i am going to|we are going to)\b",
    r"\b(decided to|choosing to|planning to|opting to|committing to)\b",
    r"\blet'?s\b",
    r"\b(need to|have to|must)\b.{0,40}\b(implement|create|build|add|use|deploy|change|update|fix)\b",
]
...
class QuantumConsciousMemory:
    ...
    def learn(self, user_message: str, ai_response: str) -> QuantumLearningResult:
        ...
        # Extract and store compound concepts (bigrams)
        ...
        for compound in compounds:
            if compound.lower() not in self.entity_cache:
                addr = self.brain.store_concept(compound, activation=0.6)
    ...
    def _extract_decisions(self, user_message: str, ai_response: str) -> int:
        """Count decision-indicating phrases across a message exchange."""
    ...
    def _extract_compound_concepts(self, text: str) -> List[str]:
        """Extract meaningful bigram compound concepts from text.

        Finds adjacent non-stop-word pairs that together name a concept
        (e.g. "quantum memory", "neural network", "activation score").
        """
