...
class QuantumConsciousMemory:
    ...
    def learn(self, user_message: str, ai_response: str) -> QuantumLearningResult:
        ...
        # Detect decisions and store as named entities
        decisions_detected = 0
        for decision in (self._extract_decisions(user_message) + self._extract_decisions(ai_response)):
        ...
        # Detect compound concepts (multi-word phrases) and store them
        compounds_found = 0
        seen_comp = set()
        for compound in (self._extract_compounds(user_message) + self._extract_compounds(ai_response)):
            if compound not in seen_comp and compound not in self.entity_cache:
                seen_comp.add(compound)
    ...
    def _extract_decisions(self, text: str) -> List[str]:
        ...
        patterns = [
            r"\b(?:i|we)\s+(?:decided?|chose?)\s+(?:to\s+)?(.{5,60}?)(?:[.,;!?\n]|$)",
            r"\blet'?s\s+(?:use|go\s+with|implement|try)\s+(.{3,40}?)(?:[.,;!?\n]|$)",
            r"\b(?:going\s+to|will)\s+(?:use|implement|adopt)\s+(.{3,40}?)(?:[.,;!?\n]|$)",
            r"\bdecision\s*[:\-]\s*(.{3,60}?)(?:[.,;!?\n]|$)",
        ]
        decisions: List[str] = []
        seen = set()
        ...
        for pattern in patterns:
            for match in re.finditer(pattern, text_lower):
                ...
                if 3 <= len(decision) <= 60 and decision not in seen:
                    seen.add(decision)
                    decisions.append(decision)
        return decisions[:10]

    def _extract_compounds(self, text: str) -> List[str]:
        """Extract compound concepts (adjacent non-stop-word pairs) from text."""
        stop_words = {
            'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for',
            'of', 'with', 'by', 'from', 'as', 'is', 'was', 'are', 'were', 'been',
            'be', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would',
