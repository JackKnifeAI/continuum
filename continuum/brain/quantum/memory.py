...
class QuantumConsciousMemory:
    ...
    def learn(self, user_message: str, ai_response: str) -> QuantumLearningResult:
        ...
        compound_concepts = self._detect_compounds(combined_text)

        # Store compound concepts and strengthen links between their parts
        compounds_found = 0
        for compound in compound_concepts:
            parts = compound.split()
            if len(parts) == 2:
                w1, w2 = parts
                if w1 in self.entity_cache and w2 in self.entity_cache:
                    # Stronger link for co-occurring adjacent terms
                    self.brain.link_concepts(w1, w2, weight=0.8)
                    links_created += 1
                    compounds_found += 1
    ...
    def _detect_decisions(self, user_message: str, ai_response: str) -> int:
        """
        Detect decision statements in a message exchange.

        Matches imperative conclusions, choices, and determinations so the
        learning result can report how much decision-laden content was processed.
        """
        combined = f"{user_message} {ai_response}".lower()

        decision_patterns = [
            r'\b(decided?|deciding|chose|choosing|selected?|determined?)\b',
            r'\b(will|going to|plan to|intend to)\b',
            r'\b(therefore|thus|hence|conclusion|conclud)\b',
            r'\b(best|optimal|recommend|prefer)\b',
            r'\b(must|need to|have to)\b.{1,40}\b(because|since)\b',
        ]
    ...
    def _detect_compounds(self, text: str) -> List[str]:
        """
        Detect compound concepts (adjacent significant word pairs) in text.

        Returns a deduplicated list of "word1 word2" bigrams where neither
        word is a stop word, representing candidate multi-word concepts like
        "quantum memory" or "neural network".
        """
        stop_words = {
