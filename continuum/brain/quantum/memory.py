...
_DECISION_PATTERNS: List[re.Pattern] = [
    re.compile(r"\bi(?:'ll| will| decided| choose| chose| plan)\b", re.IGNORECASE),
    re.compile(r"\bwe(?:'ll| will| decided| choose| chose| plan)\b", re.IGNORECASE),
    re.compile(r"\blet'?s\b", re.IGNORECASE),
    re.compile(r"\bdecided\s+to\b", re.IGNORECASE),
    re.compile(r"\bgoing\s+to\b", re.IGNORECASE),
    re.compile(r"\bwill\s+use\b", re.IGNORECASE),
    re.compile(r"\bthe\s+(?:plan|decision|choice|solution)\s+is\b", re.IGNORECASE),
]
...
class QuantumConsciousMemory:
    ...
    def learn(self, user_message: str, ai_response: str) -> QuantumLearningResult:
        ...
        for compound in raw_compounds:
            if compound not in self.entity_cache:
                ...
                compounds_found += 1
                # Link compound to its component single-word concepts
    ...
    def _extract_decisions(self, text: str) -> int:
        """Count decision statements in text using commitment-phrase patterns."""
        return sum(len(p.findall(text)) for p in _DECISION_PATTERNS)

    def _extract_compounds(self, text: str) -> List[str]:
        """Extract compound concepts as adjacent non-stopword bigrams."""
        words = re.findall(r'\b[a-zA-Z]{3,}\b', text.lower())
        seen: set = set()
        compounds: List[str] = []
