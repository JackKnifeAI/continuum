...
class QuantumConsciousMemory:
    ...
    def learn(self, user_message: str, ai_response: str) -> QuantumLearningResult:
        ...
        compounds = self._extract_compounds(all_text)
    ...
    def _extract_decisions(self, text: str) -> List[str]:
        """Detect decision statements using intent/commitment patterns."""
        patterns = [
            r"(?:i|we)\s+(?:will|shall|am\s+going\s+to|are\s+going\s+to)\s+([a-z]+(?:\s+[a-z]+){0,3})",
            r"(?:decided|choosing|chose|selected|agreed|committed)\s+(?:to\s+)?([a-z]+(?:\s+[a-z]+){0,3})",
            r"(?:let[''`]?s|let\s+us)\s+([a-z]+(?:\s+[a-z]+){0,3})",
            r"(?:i[''`]?ll|we[''`]?ll)\s+([a-z]+(?:\s+[a-z]+){0,3})",
            r"(?:the\s+)?(?:decision|choice|plan)\s+is\s+(?:to\s+)?([a-z]+(?:\s+[a-z]+){0,3})",
        ]
        decisions = []
    ...
    def _extract_compounds(self, text: str) -> List[str]:
        """Detect compound concepts — adjacent significant word pairs."""
        stop_words = {
            'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for',
            'of', 'with', 'by', 'from', 'as', 'is', 'was', 'are', 'were', 'been',
            'be', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would',
