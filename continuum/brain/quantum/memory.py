...
class QuantumConsciousMemory:
    ...
    def learn(self, user_message: str, ai_response: str) -> QuantumLearningResult:
        ...
        # Decision extraction
        decisions_detected = self._extract_decisions(f"{user_message} {ai_response}")

        # Compound concept detection and storage
        compounds = self._extract_compounds(f"{user_message} {ai_response}")
    ...
    def _extract_decisions(self, text: str) -> int:
        """Count decision indicator phrases in text."""
        matches = re.findall(
            r'\b(?:decided|choosing|chosen|will use|going with|opted for|'
            r"let'?s|planned? to|agreed to|resolved to|"
            r'conclusion|therefore|thus)\b',
            text, re.IGNORECASE
        )
        return len(matches)

    def _extract_compounds(self, text: str) -> List[str]:
        """Detect compound concepts as adjacent non-stopword bigrams."""
        stop_words = {
            'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for',
            'of', 'with', 'by', 'from', 'as', 'is', 'was', 'are', 'were', 'been',
            'be', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would',
