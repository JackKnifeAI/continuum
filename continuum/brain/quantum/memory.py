...
class QuantumConsciousMemory:
    ...
    def _extract_decisions(self, text: str) -> List[str]:
        """
        Detect decision-making statements in text using regex patterns.

        Returns a deduplicated list of matched decision phrases (up to 10).
        """
        patterns = [
            r"\b(decided|deciding)\s+to\b",
            r"\b(will|shall)\s+(?:now\s+)?(?:\w+\s+){0,3}\b(?:use|implement|build|create|deploy|remove|replace|switch|migrate|add|drop|change|update|refactor)\b",
            r"\b(chose|chosen|choosing)\s+to\b",
            r"\b(going|plan(?:ning)?)\s+to\s+(?:\w+\s+){0,2}\b(?:use|implement|build|create|deploy|remove|replace|switch|migrate|add|drop|change|update|refactor)\b",
            r"\b(?:the\s+)?(?:decision|choice|plan)\s+is\s+to\b",
            r"\blet'?s\s+(?:go\s+with|use|implement|choose)\b",
            r"\b(?:we|i)\s+(?:should|must|need\s+to)\s+(?:\w+\s+){0,2}\b(?:use|implement|build|create|deploy|remove|replace)\b",
        ]
    ...
    def _extract_compound_concepts(self, text: str, min_freq: int = 1) -> List[str]:
        """
        Extract multi-word compound concepts (bigrams of meaningful words).

        Algorithm:
        1. Tokenize using same pattern as _extract_concepts
        2. Filter through the stop_words set
        3. Form bigrams from adjacent filtered tokens
        4. Return bigrams with frequency >= min_freq, sorted by frequency desc
        ...
        freq: Dict[str, int] = {}
        for t1, t2 in zip(filtered, filtered[1:]):
            bigram = t1 + " " + t2
            freq[bigram] = freq.get(bigram, 0) + 1

        candidates = [(bigram, count) for bigram, count in freq.items() if count >= min_freq]
        candidates.sort(key=lambda x: -x[1])

        return [bigram for bigram, _ in candidates[:15]]
