...
class KMSEncryptionHandler:
    ...
    def _get_azure_kms_client(self):
        ...
        if self._kms_client is None:
            ...
            if not key_url or not key_url.startswith("https://") or ".vault.azure.net/keys/" not in key_url:
                raise ValueError(
                    "kms_key_id must be a full Azure Key Vault key URL for Azure provider. "
