...
class KMSEncryptionHandler:
    ...
    def _get_azure_kms_client(self):
        ...
        if self._kms_client is None:
            ...
            if not key_url:
                raise ValueError(
                    "kms_key_id must be set to a full Azure Key Vault key URL: "
            ...
            if not key_url.startswith("https://") or ".vault.azure.net/keys/" not in key_url:
                raise ValueError(
                    f"kms_key_id '{key_url}' is not a valid Azure Key Vault key URL. "
                    "Expected format: https://<vault-name>.vault.azure.net/keys/<key-name>[/<version>]"
                )
