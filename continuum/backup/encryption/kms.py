    ...
    def _get_azure_kms_client(self):
        ...
        if self._kms_client is None:
            if not self.config.kms_key_id:
                raise ValueError(
                    "kms_key_id must be set to the full Azure Key Vault key URL, e.g. "
                    "https://<vault>.vault.azure.net/keys/<key-name>/<key-version>"
                )
            if not self.config.kms_key_id.startswith("https://") or ".vault.azure.net/" not in self.config.kms_key_id:
                raise ValueError(
                    f"kms_key_id '{self.config.kms_key_id}' is not a valid Azure Key Vault key URL. "
                    "Expected format: https://<vault>.vault.azure.net/keys/<key-name>/<key-version>"
                )
