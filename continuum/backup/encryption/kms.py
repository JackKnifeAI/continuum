...
class KMSEncryptionHandler:
    ...
    def _get_azure_kms_client(self):
        ...
        if self._kms_client is None:
            ...
            if not key_url:
                raise ValueError(
                    "kms_key_id must be set for Azure Key Vault. "
                    "Provide the full key URL: "
                    "https://<vault-name>.vault.azure.net/keys/<key-name>/<key-version>"
                )
            if not key_url.startswith("https://"):
                raise ValueError(
                    f"kms_key_id for Azure Key Vault must be a full key URL "
                    f"(e.g. https://<vault>.vault.azure.net/keys/<key>/<version>), "
                    f"got: {key_url!r}"
                )
