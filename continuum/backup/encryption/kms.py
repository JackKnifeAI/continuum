...
class KMSEncryptionHandler:
    ...
    def _get_azure_kms_client(self):
        ...
        if self._kms_client is None:
            ...
            if not key_url:
                raise ValueError(
                    "kms_key_id must be the Azure Key Vault key URL "
                    "(e.g., https://<vault>.vault.azure.net/keys/<key-name>/<version>)"
                )
            if not key_url.startswith("https://") or ".vault.azure.net/keys/" not in key_url:
                raise ValueError(
                    f"Invalid Azure Key Vault key URL: {key_url!r}. "
                    "Expected: https://<vault-name>.vault.azure.net/keys/<key-name>/<version>"
                )
