...
_KEY_STORE_DEFAULT = Path("continuum_data/keys")


class SecureKeyStore:
    """Filesystem-based secure key store.

    Stores AES-256 keys as raw binary files under ``store_path``, one file per
    key_id.  Each file is created with mode 0o600 so only the owning process
    can read it.
    """

    def __init__(self, store_path: Optional[Path] = None):
        self._path = store_path or _KEY_STORE_DEFAULT
        self._path.mkdir(parents=True, exist_ok=True)

    def _key_file(self, key_id: str) -> Path:
        # Sanitise key_id so it cannot escape the store directory
    ...
    def load(self, key_id: str) -> Optional[bytes]:
        ...
        if mode & 0o077:
            logger.warning("Key file %s has world/group-readable permissions (%o)", key_file, mode)
