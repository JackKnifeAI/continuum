...
class AESEncryptionHandler:
    ...
    def _key_store_dir(self) -> Path:
        ...
        key_dir = Path(os.environ.get("CONTINUUM_KEY_STORE", "continuum_data/keys"))
    ...
    def _save_key(self, key_id: str, key: bytes) -> None:
        ...
        key_path = self._key_store_dir() / f"{key_id}.key"
        key_path.write_bytes(key)
        key_path.chmod(0o600)
        logger.info(f"Saved encryption key '{key_id}' to key store")
    ...
    def _load_key(self, key_id: str) -> bytes:
        ...
        if not key_path.exists():
            raise FileNotFoundError(
                f"Encryption key '{key_id}' not found in key store at {key_path.parent}. "
                "Set CONTINUUM_KEY_STORE to the correct path or generate a key first."
            )
