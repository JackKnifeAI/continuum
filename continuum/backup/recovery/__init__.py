...
async def selective_restore(
    ...
    try:
        # Look up metadata from the metadata store
...
async def _selective_restore_from_full(
    ...
    def _restore() -> tuple[int, int]:
        ...
        fd, tmp_path = tempfile.mkstemp(suffix=".db", dir=str(config.temp_dir))
        try:
            ...
            for table in to_restore:
                ...
                if target.overwrite:
                    ...
                    create_if_not_exists = create_sql.replace(
                        "CREATE TABLE", "CREATE TABLE IF NOT EXISTS", 1
                    )
                    dst_cur.execute(create_if_not_exists)
...
async def _selective_restore_from_incremental(
    ...
    """Apply incremental JSON changes for specific tables only."""
    changes = json.loads(backup_data.decode("utf-8"))
