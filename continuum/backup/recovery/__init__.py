...
async def selective_restore(
    ...
    try:
        ...
        result.success = True
        logger.info(
            f"Selective restore complete: {result.records_restored} records "
...
async def _selective_restore_from_full(
    ...
    """Copy only the requested tables from a full (SQLite binary) backup."""

    def _do_restore():
        config.temp_dir.mkdir(parents=True, exist_ok=True)
        fd, tmp_path = tempfile.mkstemp(dir=str(config.temp_dir), suffix=".db")
        try:
            ...
            for table in to_restore:
                ...
                # CREATE TABLE IF NOT EXISTS in the destination
...
async def _selective_restore_from_incremental(
    ...
    """Apply only the requested tables' changes from an incremental/differential backup."""
    changes = json.loads(backup_data.decode("utf-8"))
    ...
    def _apply():
        ...
        conn.commit()
        conn.close()
        return tables_updated, total_records

    tables_count, records_count = await asyncio.to_thread(_apply)
    result.tables_restored = tables_count
    result.records_restored = records_count
    logger.info(f"Applied selective incremental changes: {records_count} records across {tables_count} tables")
