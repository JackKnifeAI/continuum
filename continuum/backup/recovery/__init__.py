...
async def selective_restore(
    ...
    try:
        # Load backup metadata to know encryption/compression state
        ...
        # Warn about tables not present in backup, restore only those that exist
        if metadata.tables:
            missing = set(tables) - set(metadata.tables)
            if missing:
                result.warnings.append(f"Tables not in backup: {sorted(missing)}")
            tables_to_restore = [t for t in tables if t in metadata.tables]
        else:
            tables_to_restore = tables

        if not tables_to_restore:
            result.error = "None of the requested tables exist in this backup"
            result.status = RestoreStatus.FAILED
            return result
        ...
        result.status = RestoreStatus.RESTORING
        if metadata.strategy.value == 'full':
            await _restore_selected_tables_from_full(backup_data, tables_to_restore, target, result, config)
        else:
            await _restore_selected_tables_from_incremental(backup_data, tables_to_restore, target, result)
...
async def _restore_selected_tables_from_full(
    ...
    """Copy specific tables from a full SQLite backup to the target database."""
    if not target.database_path:
        raise ValueError("database_path required for restore")
    ...
    def _extract_and_copy():
        fd, temp_path = tempfile.mkstemp(dir=str(config.temp_dir), suffix='.db')
        os.close(fd)
        try:
            ...
            tables_to_copy = [t for t in tables if t in available]
            ...
            for table in tables_to_copy:
                ...
                col_count = len(src_cursor.fetchall())
            ...
            dst_conn.close()
            src_conn.close()
            return len(tables_to_copy), total_records
        finally:
            os.unlink(temp_path)

    tables_count, records_count = await asyncio.to_thread(_extract_and_copy)
    result.tables_restored = tables_count
    result.records_restored = records_count
    logger.info(f"Selectively restored {tables_count} tables, {records_count} records")
...
async def _restore_selected_tables_from_incremental(
    ...
    filtered = {
        name: data
        for name, data in changes.get('tables', {}).items()
    ...
    def _apply():
        ...
        for table_name, table_changes in filtered.items():
            rows = table_changes.get('rows', [])
            for row in rows:
                columns = list(row.keys())
                placeholders = ','.join(['?' for _ in columns])
    ...
    records_restored = await asyncio.to_thread(_apply)
    result.records_restored = records_restored
    result.tables_restored = len(filtered)
    logger.info(f"Applied selective incremental changes: {records_restored} records across {len(filtered)} tables")
