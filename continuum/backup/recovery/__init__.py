...
async def selective_restore(
    ...
    try:
        # Fetch backup metadata to know encryption/compression state
...
async def _selective_restore_from_full(
    ...
    temp_path = config.temp_dir / f"selective_{_dt.utcnow().strftime('%Y%m%d_%H%M%S')}.db"
    ...
    def _extract():
        ...
        try:
            ...
            for table in tables:
                ...
                # Ensure table exists in target (idempotent)
                dst_cur.execute(
                    create_sql.replace("CREATE TABLE ", "CREATE TABLE IF NOT EXISTS ", 1)
                )

                # Read all column names via PRAGMA (avoids relying on DDL parsing)
                ...
                if rows:
                    insert_mode = "INSERT" if target.overwrite else "INSERT OR IGNORE"
                    dst_cur.executemany(
                        f"{insert_mode} INTO {table} VALUES ({placeholders})",  # noqa: S608
                        rows,
                    )
                    total_records += len(rows)

                restored_tables += 1
                logger.debug(f"Restored table {table!r}: {len(rows)} rows")
    ...
    restored_tables, total_records = await asyncio.to_thread(_extract)
    result.tables_restored = restored_tables
    result.records_restored = total_records
    logger.info(f"Full-backup selective restore: {restored_tables} tables, {total_records} records")
...
async def _selective_restore_from_incremental(
    ...
    if not target.database_path:
        raise ValueError("database_path required for selective incremental restore")
    ...
    restored_tables, total_records = await asyncio.to_thread(_apply)
    result.tables_restored = restored_tables
    result.records_restored = total_records
    logger.info(
        f"Incremental selective restore: {restored_tables} tables, {total_records} records"
    )
