...
async def selective_restore(
    ...
    Returns:
        RestoreResult with status
    """
    logger.info(f"Selective restore: {tables} from backup {backup_id}")
    ...
    try:
        ...
        if metadata.strategy.value == 'full':
            # Full backup: write to temp file, then copy only requested tables
            records, tables_done = await _selective_restore_from_full(
                backup_data, tables, target
            )
        else:
            # Incremental/differential: filter JSON changes to requested tables
            records, tables_done = await _selective_restore_from_incremental(
                backup_data, tables, target
            )
        ...
        result.status = RestoreStatus.COMPLETED
        logger.info(
            f"Selective restore complete: {tables_done} tables, {records} records"
        )
...
async def _selective_restore_from_full(
    ...
    """Copy only requested tables from a full (SQLite) backup into the target DB."""
    import tempfile
    ...
    def _copy_tables() -> tuple[int, int]:
        ...
        try:
            tmp_path.write_bytes(backup_data)
            ...
            for table in tables:
                ...
                tables_done += 1
                logger.info(f"Copied {len(rows)} rows for table: {table}")

            dst.commit()
            src.close()
            dst.close()
            return total_records, tables_done
...
async def _selective_restore_from_incremental(
    ...
    """Apply only requested tables from an incremental (JSON) backup."""
    changes = json.loads(backup_data.decode("utf-8"))
    ...
    def _apply() -> tuple[int, int]:
        ...
        for table_name, table_changes in changes.get("tables", {}).items():
            ...
            tables_done += 1
            logger.info(f"Applied {len(rows)} changes for table: {table_name}")
