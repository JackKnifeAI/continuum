...
async def selective_restore(
    ...
    try:
        ...
        # Load metadata to know backup type
...
async def _selective_restore_from_full(
    ...
    def _extract_and_apply() -> tuple[int, int]:
        # Write backup to a temp file so we can open it as a SQLite database
        ...
        try:
            ...
            if not target_tables:
                src.close()
                raise ValueError(f"None of the requested tables exist in backup: {tables}")
            ...
            for table in target_tables:
                # Recreate table schema in target if missing
                src_cursor.execute(
                ...
                if not row or not row[0]:
                    logger.warning(f"No schema found for table {table!r}, skipping")
                    continue
                ...
                restored_tables += 1
                logger.info(f"Restored table {table!r}: {len(rows)} rows")
...
async def _selective_restore_from_incremental(
    ...
    """Apply only the specified tables' changes from an incremental backup."""
    changes = json.loads(backup_data.decode("utf-8"))
    ...
    if not target.database_path.exists():
        raise FileNotFoundError(
            f"Base database required for incremental selective restore: "
    ...
    def _apply() -> tuple[int, int]:
        ...
        for table_name, table_changes in changes.get("tables", {}).items():
            ...
            restored_tables += 1
            logger.info(f"Applied {len(rows)} incremental changes to {table_name!r}")
