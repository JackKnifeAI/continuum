...
async def selective_restore(
    ...
    try:
        # Fetch metadata from the metadata store
        ...
        result.status = RestoreStatus.COMPLETED
        result.success = True
        logger.info(f"Selective restore completed: {result.tables_restored} tables, {result.records_restored} records")
        return result
...
async def _selective_restore_full(
    ...
    temp_path = config.temp_dir / f"selective_{datetime.utcnow().timestamp()}.db"

    def _do_selective_restore() -> tuple[int, int]:
        config.temp_dir.mkdir(parents=True, exist_ok=True)
        temp_path.write_bytes(backup_data)
        try:
            src = sqlite3.connect(str(temp_path))
            src_cur = src.cursor()

            # Validate requested tables exist in the backup
            ...
            if missing:
                raise ValueError(f"Tables not found in backup: {missing}")
            ...
            total_records = 0
            for table in tables:
                # Get CREATE TABLE statement from the source backup
                src_cur.execute(
                ...
                # Recreate table in target (drop existing if overwrite allowed)
                if target.overwrite:
                ...
                src_cur.execute(f"PRAGMA table_info({table})")  # noqa: S608
    ...
    tables_count, records_count = await asyncio.to_thread(_do_selective_restore)
    result.tables_restored = tables_count
    result.records_restored = records_count
    ...
    def _apply_changes() -> tuple[int, int]:
        ...
        for table_name, table_changes in changes.get("tables", {}).items():
            ...
            for row in rows:
                ...
                column_names = ",".join(columns)
                query = (
                    f"INSERT OR REPLACE INTO {table_name} ({column_names}) "  # noqa: S608
            ...
            if rows:
                tables_changed.add(table_name)

        conn.commit()
        conn.close()
        return total_records, len(tables_changed)

    records, tables_count = await asyncio.to_thread(_apply_changes)
    result.records_restored = records
    result.tables_restored = tables_count
    logger.info(f"Applied {records} selective incremental changes across {tables_count} tables")
