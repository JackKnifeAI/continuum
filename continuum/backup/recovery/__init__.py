...
async def _selective_restore_from_full(
    ...
    """Extract specific tables from a full SQLite backup and copy to target database."""
    if not target.database_path:
        ...
        try:
            ...
            for table in tables:
                ...
                if target.overwrite:
                    ...
                    dst_cursor.execute(create_if_not_exists)
...
async def _selective_restore_from_incremental(
    ...
    """Apply incremental JSON changes for the specified tables only."""
    changes = json.loads(backup_data.decode('utf-8'))
