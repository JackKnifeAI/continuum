...
async def selective_restore(
    ...
    try:
        ...
        # Load metadata to know encryption/compression state
...
async def _selective_restore_full(
    ...
    def _restore_tables():
        ...
        try:
            ...
            for table_name in tables_to_restore:
                ...
                if target.overwrite:
                    dst_cursor.execute(f"DROP TABLE IF EXISTS {table_name}")  # noqa: S608

                dst_cursor.execute(schema_row[0])

                src_cursor.execute(f"SELECT * FROM {table_name}")  # noqa: S608
    ...
    result.tables_restored = tables_count
    result.records_restored = records_count
    logger.info(f"Selective full restore: {tables_count} tables, {records_count} records")
...
async def _selective_restore_incremental(
    ...
    """Apply incremental changes for the specified tables only."""
    changes = json.loads(backup_data.decode('utf-8'))
    ...
    def _apply_selective_changes():
        ...
        return len(tables_affected), total_records

    tables_count, records_count = await asyncio.to_thread(_apply_selective_changes)
    result.tables_restored = tables_count
    result.records_restored = records_count
    logger.info(f"Selective incremental restore: {records_count} changes to {tables_count} tables")
