...
async def selective_restore(
    ...
    try:
        # Look up metadata to know encryption/compression/strategy
        ...
        # Route to the appropriate extractor based on backup strategy
        is_incremental = (
            metadata is not None
            and metadata.strategy.value in ('incremental', 'differential')
        )
...
async def _selective_restore_from_full(
        ...
        try:
            ...
            for table_name in tables:
                if table_name not in available:
                    logger.warning(f"Table not found in backup, skipping: {table_name}")
                    continue
                ...
                # Fetch column names via PRAGMA so we can build the INSERT
                ...
                tables_restored += 1
                logger.debug(f"Restored table {table_name}: {len(rows)} rows")
    ...
    tables_count, records_count = await asyncio.to_thread(_restore)
    result.tables_restored = tables_count
    result.records_restored = records_count
    logger.info(f"Selectively restored {tables_count} tables, {records_count} records from full backup")
...
async def _selective_restore_from_incremental(
    ...
    """Apply changes for specific tables from an incremental/differential backup."""
    if not target.database_path:
        raise ValueError("database_path required for selective restore")
    ...
    result.records_restored = records_count
    logger.info(
        f"Selectively restored {tables_count} tables, {records_count} records "
