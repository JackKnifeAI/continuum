...
class BackupManager:
    ...
    async def _count_records(self, tables: Optional[List[str]] = None) -> int:
        """Count total records across the given tables (or all tables if None)."""
        db_path = self.config.db_path
        ...
        def _count_sync() -> int:
            ...
            try:
                ...
                for table in table_list:
                    try:
                        # Table names come from sqlite_master, not external input.
                        cursor.execute(f"SELECT COUNT(*) FROM [{table}]")  # noqa: S608
                        total += cursor.fetchone()[0]
                    except sqlite3.Error:
                        logger.warning(f"Could not count records in table {table!r}")
                return total
            finally:
                conn.close()
