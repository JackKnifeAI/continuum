...
class ContinuousBackupStrategy(BackupStrategyBase):
    ...
    def get_base_backup_id(self) -> Optional[str]:
        """Get the base full backup ID that this continuous stream builds upon."""
        return self._base_backup_id

    def set_base_backup(self, backup_id: str) -> None:
        """Set the base full backup for this continuous backup stream.

        Args:
            backup_id: ID of the full backup this stream is based on
        """
        self._base_backup_id = backup_id
        logger.info(f"Continuous backup stream base set to: {backup_id}")
