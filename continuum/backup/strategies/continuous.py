...
class ContinuousBackupStrategy(BackupStrategyBase):
    ...
    def get_base_backup_id(self) -> Optional[str]:
        """Return the base full backup ID this continuous stream builds upon."""
        return self._base_backup_id

    def set_base_backup(self, backup_id: str) -> None:
        """Set the base full backup that this continuous backup stream extends."""
        self._base_backup_id = backup_id
        logger.debug(f"Continuous backup base set to: {backup_id}")
