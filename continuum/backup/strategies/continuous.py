...
class ContinuousBackupStrategy(BackupStrategyBase):
    ...
    def get_base_backup_id(self) -> Optional[str]:
        """Get the base full backup ID that this continuous stream anchors to."""
        return self._base_backup_id

    def set_base_backup(self, backup_id: str) -> None:
        """Set the base full backup that continuous batches reference."""
        self._base_backup_id = backup_id
        logger.debug(f"Continuous strategy anchored to base backup: {backup_id}")
