...
class ContinuousBackupStrategy(BackupStrategyBase):
    ...
    def set_base_backup_id(self, backup_id: str) -> None:
        """Record the full backup this continuous stream is based on."""
        self._base_backup_id = backup_id
        logger.info(f"Base backup set: {backup_id}")

    def get_base_backup_id(self) -> Optional[str]:
        """Return the ID of the full backup this continuous stream references."""
