    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                logger.debug(f"Flushed usage cache to storage ({len(cache_snapshot)} keys)")
...
class UsageReporter:
    ...
    def unregister_subscription(self, tenant_id: str) -> None:
        """Unregister a tenant's subscription from background reporting."""
        ...
        while True:
            ...
            for tenant_id, subscription_item_id in subscriptions:
                try:
                    ...
                    logger.error(f"Failed to report usage for {tenant_id}: {e}")
