    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                # Serialize defaultdict → plain dict so the backend can persist it
...
class UsageReporter:
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            logger.debug(f"Background usage reporting tick: {len(self._subscriptions)} active subscription(s)")
