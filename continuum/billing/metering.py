    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                serializable_cache = {
                    key: dict(value) for key, value in self._usage_cache.items()
                }
                await self.storage.save_usage(serializable_cache)
                logger.debug(f"Flushed {len(serializable_cache)} usage cache entries to storage")
...
class UsageReporter:
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            logger.debug(
                f"Background usage reporting: processing {len(self._active_subscriptions)} subscriptions"
            )
