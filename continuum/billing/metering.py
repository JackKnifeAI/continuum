...
class UsageMetering:
    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                logger.debug(f"Flushing {len(self._usage_cache)} usage entries to storage")
                await self.storage.save_usage(dict(self._usage_cache))
                logger.info(f"Flushed usage cache to storage ({len(self._usage_cache)} entries)")
...
class UsageReporter:
    ...
    def __init__(
        ...
        # tenant_id -> subscription_item_id for background reporting
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            logger.debug(f"Background usage reporting for {len(self._subscriptions)} subscriptions")
