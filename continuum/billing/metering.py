...
class UsageMetering:
    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                usage_snapshot = {key: dict(metrics) for key, metrics in self._usage_cache.items()}
                await self.storage.save_usage(usage_snapshot)
                logger.debug(f"Flushed {len(usage_snapshot)} usage cache entries to storage")
...
class UsageReporter:
    ...
    def __init__(
        ...
        self._active_subscriptions: Dict[str, str] = {}  # tenant_id -> subscription_item_id
    ...
    def register_subscription(self, tenant_id: str, subscription_item_id: str) -> None:
        """Register a subscription for background usage reporting."""
        self._active_subscriptions[tenant_id] = subscription_item_id
        logger.info(f"Registered subscription {subscription_item_id} for tenant {tenant_id}")

    def unregister_subscription(self, tenant_id: str) -> None:
        """Unregister a subscription (e.g., when canceled)."""
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            logger.debug(f"Background usage reporting tick: {len(self._active_subscriptions)} active subscriptions")
