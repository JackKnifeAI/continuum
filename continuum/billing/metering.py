    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                logger.debug(f"Flushed {len(usage_snapshot)} usage entries to storage")
...
class UsageReporter:
    ...
    def register_subscription(self, tenant_id: str, subscription_item_id: str) -> None:
        """
        Register an active subscription for background reporting.
    ...
    def unregister_subscription(self, tenant_id: str) -> None:
        """
        Unregister a subscription from background reporting.
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            logger.debug(
                f"Background usage reporting tick: {len(self._subscriptions)} active subscriptions"
            )
