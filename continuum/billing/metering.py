    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                if asyncio.iscoroutinefunction(getattr(self.storage, 'save_usage', None)):
                    await self.storage.save_usage(snapshot)
                elif hasattr(self.storage, 'save_usage'):
                    self.storage.save_usage(snapshot)
                else:
                    logger.warning("Storage backend has no save_usage method; skipping flush")
...
class UsageReporter:
    ...
    def unregister_subscription(self, tenant_id: str) -> None:
        """Remove a tenant subscription from background reporting."""
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            tasks = [
                self.report_usage_to_stripe(tenant_id, subscription_item_id)
                for tenant_id, subscription_item_id in list(self._subscriptions.items())
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for (tenant_id, _), result in zip(self._subscriptions.items(), results):
