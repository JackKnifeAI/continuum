    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                usage_snapshot = {k: dict(v) for k, v in self._usage_cache.items()}
...
class UsageReporter:
    ...
    def __init__(
        ...
        # tenant_id -> subscription_item_id for active metered subscriptions
        self._active_subscriptions: Dict[str, str] = {}

    def register_subscription(self, tenant_id: str, subscription_item_id: str) -> None:
        """
        Register a tenant's active metered subscription for background reporting.

        Args:
            tenant_id: Tenant identifier
            subscription_item_id: Stripe subscription item ID (the metered item)
        """
