    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                elif hasattr(self.storage, 'set_many'):
                    await self.storage.set_many(dict(self._usage_cache))
                else:
                    logger.warning("Storage backend does not support bulk save; skipping flush")
