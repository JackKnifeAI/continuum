    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                logger.debug("Flushed usage cache to storage")
...
class UsageReporter:
    ...
    def register_subscription(self, tenant_id: str, subscription_item_id: str) -> None:
        ...
        Args:
            tenant_id: Tenant identifier
            subscription_item_id: Stripe subscription item ID to bill against
        """
        self._active_subscriptions[tenant_id] = subscription_item_id
        logger.debug(f"Registered subscription item {subscription_item_id} for tenant {tenant_id}")

    def unregister_subscription(self, tenant_id: str) -> None:
        """
        Unregister a tenant from background usage reporting.

        Args:
            tenant_id: Tenant identifier
        """
        self._active_subscriptions.pop(tenant_id, None)
        logger.debug(f"Unregistered subscription reporting for tenant {tenant_id}")
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            await self._report_all_subscriptions()

    async def _report_all_subscriptions(self) -> None:
        """Report usage for every registered active subscription."""
        if not self._active_subscriptions:
            logger.debug("Background usage reporting tick: no active subscriptions")
            return

        logger.debug(
            f"Background usage reporting tick: {len(self._active_subscriptions)} subscriptions"
        )
