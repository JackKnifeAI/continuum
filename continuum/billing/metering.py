...
class UsageReporter:
    ...
    def register_subscription(self, tenant_id: str, subscription_item_id: str) -> None:
        ...
        logger.debug(f"Registered subscription for background reporting: {tenant_id}")

    def unregister_subscription(self, tenant_id: str) -> None:
        """
        Unregister a tenant from background reporting (e.g., on cancellation).

        Args:
            tenant_id: Tenant identifier
        """
        self._active_subscriptions.pop(tenant_id, None)
        logger.debug(f"Unregistered subscription from background reporting: {tenant_id}")
