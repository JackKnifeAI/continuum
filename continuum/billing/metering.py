...
class UsageMetering:
    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                usage_data = {key: dict(metrics) for key, metrics in self._usage_cache.items()}
                await self.storage.save_usage(usage_data)
                logger.debug(f"Flushed {len(usage_data)} usage records to storage")
...
class UsageReporter:
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            logger.debug(f"Background usage reporting tick: {len(active)} subscriptions")
