    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                ...
                cache_snapshot = {
                    key: dict(metrics)
                    for key, metrics in self._usage_cache.items()
                }
                await self.storage.save_usage(cache_snapshot)
                logger.debug(f"Flushed {len(cache_snapshot)} usage records to storage")
...
class UsageReporter:
    ...
    def register_subscription(self, tenant_id: str, subscription_item_id: str) -> None:
        ...
        logger.debug(f"Registered subscription for reporting: {tenant_id} -> {subscription_item_id}")
    ...
    def unregister_subscription(self, tenant_id: str) -> None:
        ...
        logger.debug(f"Unregistered subscription for reporting: {tenant_id}")
