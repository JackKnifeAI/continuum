...
class UsageMetering:
    ...
    async def _flush_cache(self) -> None:
        ...
        if self.storage:
            try:
                logger.debug(f"Flushing usage cache to storage ({len(self._usage_cache)} entries)")
                await self.storage.save_usage(self._usage_cache)
                self._usage_cache = defaultdict(lambda: defaultdict(int))
    ...
    async def start_background_reporting(self) -> None:
        ...
        while True:
            ...
            if not self._subscriptions:
                logger.debug("Background usage reporting tick: no subscriptions registered")
                continue
