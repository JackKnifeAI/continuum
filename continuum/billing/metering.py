        ...
        if self.storage:
            try:
                ...
                self.storage.execute(
                    ...
                    "(cache_key TEXT NOT NULL, metric TEXT NOT NULL, "
                    "value INTEGER NOT NULL, flushed_at TEXT NOT NULL, "
                ...
                if rows:
                    self.storage.executemany(
                        ...
                        "(cache_key, metric, value, flushed_at) VALUES (?, ?, ?, ?)",
                        rows
                    )
                    logger.debug(f"Flushed {len(rows)} usage metric rows to storage")
