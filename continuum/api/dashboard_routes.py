...
# Global metering instance (shared with server.py)
# This will be injected by the server when the router is mounted
_metering_instance: Optional[UsageMetering] = None


def set_metering_instance(metering: UsageMetering) -> None:
    """
    Set the global metering instance for this router.

    Called by server.py during initialization to inject the shared
    metering instance used for rate limiting and usage tracking.
...
def get_metering_instance() -> UsageMetering:
    ...
    if _metering_instance is None:
        # Fallback: create a new instance if not injected
        # This allows the router to work independently for testing
