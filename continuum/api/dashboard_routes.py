...
# Module-level metering singleton.  In production the server wires its own
# UsageMetering instance via BillingMiddleware; this instance serves the
# dashboard endpoint and can be replaced by the application factory to share
# the same in-memory state as the middleware.
...
async def get_dashboard_stats(
    ...
    try:
        ...
        # Query today's API call count from the metering system.
        api_calls_today = await usage_metering.get_usage(
            tenant_id, "api_calls", period="day"
        )
