...
def is_legacy_hash(stored_hash: str) -> bool:
    """Detect old SHA-256 format: 64 hex chars, no colon separator."""
...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    try:
        ...
        # Legacy SHA-256 fallback; validate_api_key upgrades these hashes on successful auth
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            # Opportunistically migrate legacy SHA-256 hashes to PBKDF2
