...
def verify_key(key: str, stored_hash: str) -> tuple[bool, bool]:
    ...
    Supports both PBKDF2-HMAC-SHA256 (current) and plain SHA-256 (legacy).
    Returns a second flag signalling that the stored hash should be upgraded.

    Args:
        key: Plain text API key to verify
        stored_hash: Either ``salt_hex:hash_hex`` (PBKDF2) or a bare hex digest (legacy)

    Returns:
        ``(is_valid, needs_upgrade)`` — needs_upgrade is True only for legacy hashes
        that matched, prompting the caller to re-hash and persist the new value.
    """
    try:
        ...
        # Legacy plain SHA-256 hash — verify and signal that the caller should upgrade it.
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        is_valid, needs_upgrade = verify_key(key, stored_hash)
        if is_valid:
            now = datetime.now().isoformat()
            if needs_upgrade:
                # Transparently migrate legacy SHA-256 hash to PBKDF2 on first use.
