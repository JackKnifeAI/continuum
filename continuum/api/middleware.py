...
def is_legacy_hash(stored_hash: str) -> bool:
    """Return True if stored_hash is a legacy bare SHA-256 (no salt prefix)."""
...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    try:
        ...
        # Fallback for legacy bare SHA-256 hashes. validate_api_key automatically
        # rehashes matching keys to PBKDF2, so this path becomes dead code over time.
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if is_legacy_hash(stored_hash):
                # Auto-migrate: replace bare SHA-256 hash with PBKDF2 on first use.
