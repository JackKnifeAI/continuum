...
def _is_legacy_hash(stored_hash: str) -> bool:
    """Return True if stored_hash is an old plain SHA-256 hex digest (no salt separator)."""
...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    if _is_legacy_hash(stored_hash):
        # Legacy SHA-256 path — validate_api_key upgrades the stored hash on success
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if _is_legacy_hash(stored_hash):
                # Lazy migration: upgrade SHA-256 hash to PBKDF2 on first successful use
