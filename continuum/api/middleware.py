...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    if is_legacy_hash(stored_hash):
        # Fallback for old unsalted SHA-256 hashes.
        # validate_api_key upgrades matching keys to PBKDF2 on each successful
        # login so this branch becomes dead code once all keys are migrated.
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if is_legacy_hash(stored_hash):
                # Upgrade to PBKDF2 on the fly so the legacy branch is never
                # reached again for this key after migration.
