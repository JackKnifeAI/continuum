...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    try:
        ...
        # Fallback for legacy SHA-256 hashes (no salt separator).
        # validate_api_key auto-upgrades these to PBKDF2 on successful login.
...
def is_legacy_hash(stored_hash: str) -> bool:
    """Return True if the hash uses the old plain SHA-256 format (no salt separator)."""
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if is_legacy_hash(stored_hash):
                # Auto-migrate: replace legacy SHA-256 hash with PBKDF2 in one UPDATE
