...
def verify_key(key: str, stored_hash: str) -> bool:
    """
    Verify API key against stored PBKDF2 hash or legacy SHA-256 hash.

    Args:
        key: Plain text API key to verify
        stored_hash: PBKDF2 hash (salt_hex:hash_hex) or legacy SHA-256 hex string

    Returns:
        True if key matches, False otherwise
    """
    if is_legacy_hash(stored_hash):
        # Legacy SHA-256 path — validate_api_key migrates on successful auth
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if is_legacy_hash(stored_hash):
                # Opportunistically migrate SHA-256 key to PBKDF2 on first successful use.
                # Once every key has been authenticated at least once, the legacy path in
                # verify_key can be removed.
