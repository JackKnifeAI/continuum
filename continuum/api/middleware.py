...
def verify_key(key: str, stored_hash: str) -> tuple[bool, bool]:
    ...
    Returns:
        Tuple of (is_valid, is_legacy_format). Callers should re-hash and persist
        the key when is_legacy_format is True so the database migrates on use.
    """
    try:
        ...
        # Fallback for legacy plain SHA-256 hashes; caller will auto-migrate on match.
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        ...
        if is_valid:
            ...
            if is_legacy:
                # Auto-migrate: replace legacy SHA-256 hash with PBKDF2 in place.
