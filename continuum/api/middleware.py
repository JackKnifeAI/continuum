...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    try:
        ...
        # Fallback for legacy plain SHA-256 hashes (no salt, no iterations).
        # Auto-migration in validate_api_key() rehashes on successful login,
        # so this branch becomes unreachable once all stored hashes contain ':'.
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if ':' not in stored_hash:
                # Migrate legacy SHA-256 hash to PBKDF2 in-place.
