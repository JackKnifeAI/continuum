...
def verify_key(key: str, stored_hash: str) -> tuple[bool, bool]:
    ...
    Args:
        ...
        stored_hash: Stored hash in format salt_hex:hash_hex, or legacy SHA-256 hex

    Returns:
        Tuple of (is_valid, is_legacy_format). Callers should upgrade legacy hashes.
    """
    try:
        ...
        # Fallback for old plain SHA-256 hashes stored before PBKDF2 migration.
        # validate_api_key() re-hashes and overwrites these on first use so this
        # branch naturally empties out; remove once no legacy rows remain.
        old_hash = hashlib.sha256(key.encode()).hexdigest()
        return hmac.compare_digest(old_hash, stored_hash), True
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        is_valid, is_legacy = verify_key(key, stored_hash)
        if is_valid:
            now = datetime.now().isoformat()
            if is_legacy:
                # Transparently upgrade the stored hash to PBKDF2 on first use,
                # so legacy rows drain naturally without a one-time migration script.
