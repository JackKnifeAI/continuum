...
def is_legacy_hash(stored_hash: str) -> bool:
    """Return True if the hash uses the old plain SHA-256 format (no salt prefix)."""
...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    Also accepts legacy plain SHA-256 hashes; callers should migrate on match
    (see validate_api_key).
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if is_legacy_hash(stored_hash):
                # Migrate SHA-256 hash to PBKDF2 on first use
