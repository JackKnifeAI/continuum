...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    Args:
        ...
        stored_hash: Stored hash in format salt_hex:hash_hex, or legacy plain SHA-256 hex

    Returns:
        True if key matches, False otherwise
    """
    if is_legacy_hash(stored_hash):
        # Legacy SHA-256 path — removed once migrate_key_hash() has run for all rows
...
def migrate_key_hash(key: str, stored_hash: str, conn: sqlite3.Connection) -> str:
    """
    Upgrade a legacy SHA-256 hash to PBKDF2 in the database.

    Called lazily on successful validation so active keys self-migrate on next use.
    Once all rows use the salt:hash format, verify_key's legacy branch can be removed.

    Args:
        key: Plain text API key (needed to re-hash with PBKDF2)
        stored_hash: Current legacy hash to replace
        conn: Open database connection

    Returns:
        The new PBKDF2 hash that now lives in the database
    """
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            # Lazily upgrade legacy SHA-256 hashes to PBKDF2 on first successful use
            if is_legacy_hash(stored_hash):
                stored_hash = migrate_key_hash(key, stored_hash, conn)
