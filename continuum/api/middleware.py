...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    try:
        ...
        # Fallback for old SHA-256 hashes (no salt:hash separator).
        # validate_api_key re-hashes to PBKDF2 on match; remove once
        # all keys are confirmed migrated.
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if ':' not in stored_hash:
                # Auto-migrate old SHA-256 hash to PBKDF2 on first use
