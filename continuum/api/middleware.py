...
def _is_legacy_hash(stored_hash: str) -> bool:
    """Return True if stored_hash is legacy plain-SHA-256 format (no salt separator)."""
...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    Args:
        ...
        stored_hash: Stored hash in format salt_hex:hash_hex (or legacy plain SHA-256)
    ...
    try:
        ...
        # Fallback for legacy SHA-256 hashes — auto-upgrade happens in validate_api_key
        # on successful match, so this branch becomes dead code once all keys are migrated.
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if _is_legacy_hash(stored_hash):
                # Upgrade-on-read: replace legacy SHA-256 hash with PBKDF2
