...
def is_legacy_hash(stored_hash: str) -> bool:
    """Return True if stored_hash uses legacy plain SHA-256 format (no salt separator)."""
    return ':' not in stored_hash


def verify_key(key: str, stored_hash: str) -> bool:
    """
    Verify API key against stored hash (PBKDF2 or legacy SHA-256).

    Args:
        key: Plain text API key to verify
        stored_hash: PBKDF2 format (salt_hex:hash_hex) or legacy plain SHA-256
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            if is_legacy_hash(stored_hash):
                # Auto-migrate legacy SHA-256 hash to PBKDF2 on next use
