...
def _is_pbkdf2_hash(stored_hash: str) -> bool:
    """Return True if stored_hash is in PBKDF2 format (salt_hex:hash_hex)."""
    parts = stored_hash.split(':')
    return len(parts) == 2 and len(parts[0]) == 64  # 32-byte salt = 64 hex chars


def verify_key(key: str, stored_hash: str) -> bool:
    """
    Verify API key against stored PBKDF2 hash, with fallback for legacy SHA-256.

    Args:
        key: Plain text API key to verify
        stored_hash: Stored hash — either PBKDF2 (salt_hex:hash_hex) or legacy SHA-256 hex
    ...
    try:
        ...
        # Legacy SHA-256 fallback — keys are re-hashed to PBKDF2 on next use
        # by validate_api_key, so this path disappears as keys are rotated.
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if not _is_pbkdf2_hash(stored_hash):
                # Opportunistically migrate legacy SHA-256 hash to PBKDF2
