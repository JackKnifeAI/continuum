...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    Also accepts the legacy plain SHA-256 format so that keys can be verified
    and then auto-migrated by the caller (validate_api_key).

    Args:
        key: Plain text API key to verify
        stored_hash: Hash in PBKDF2 format (salt_hex:hash_hex) or legacy SHA-256 hex
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if is_legacy_hash(stored_hash):
                # Auto-migrate: replace old SHA-256 hash with PBKDF2 on first successful login
