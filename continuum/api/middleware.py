...
def _is_legacy_hash(stored_hash: str) -> bool:
    """Return True if this is a pre-migration plain SHA-256 hash (no salt prefix)."""
...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    try:
        ...
        # Fallback for legacy plain SHA-256 hashes. validate_api_key() auto-migrates
        # any matched legacy key to PBKDF2 on first use. Remove this block once
        # no ':'-free hashes remain in the DB.
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if _is_legacy_hash(stored_hash):
                # Auto-migrate: upgrade legacy SHA-256 hash to PBKDF2 on first use
