...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    try:
        ...
        # Fallback for legacy plain SHA-256 hashes; auto-migrated to PBKDF2
        # in validate_api_key() on first successful login after this path fires.
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            # Auto-migrate legacy plain SHA-256 hashes to PBKDF2 on first successful login.
            # Once all rows contain a ':' separator the fallback in verify_key() is unreachable.
