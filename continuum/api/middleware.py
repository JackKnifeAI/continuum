...
def verify_key(key: str, stored_hash: str) -> tuple[bool, bool]:
    ...
    Args:
        ...
        stored_hash: Either PBKDF2 hash (salt_hex:hash_hex) or legacy SHA-256 hex

    Returns:
        Tuple of (is_valid, is_legacy) where is_legacy=True signals the caller
        should migrate the stored hash to PBKDF2 format.
    """
    try:
        ...
        # Fallback for legacy SHA-256 hashes — caller will migrate on success
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        ...
        if is_valid:
            ...
            if is_legacy:
                # Migrate legacy SHA-256 hash to PBKDF2 on first successful use
