...
def is_legacy_hash(stored_hash: str) -> bool:
    """Return True if stored_hash is in the legacy bare-SHA-256 format (no salt separator)."""
...
def verify_key(key: str, stored_hash: str) -> bool:
    ...
    Args:
        ...
        stored_hash: Stored hash in format salt_hex:hash_hex (or legacy bare SHA-256)

    Returns:
        True if key matches, False otherwise
    """
    if is_legacy_hash(stored_hash):
        # Legacy SHA-256 path — only reached until validate_api_key upgrades the row.
        # Remove once no rows without ':' remain in api_keys.
...
def validate_api_key(key: str) -> Optional[str]:
    ...
    for stored_hash, tenant_id in rows:
        if verify_key(key, stored_hash):
            ...
            if is_legacy_hash(stored_hash):
                # Upgrade the stored hash from bare SHA-256 to PBKDF2 in-place.
                # Once all rows are upgraded this branch becomes unreachable and
                # the legacy path in verify_key can be removed.
